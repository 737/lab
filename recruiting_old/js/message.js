...
!function($){
    ...
    wcm_message.setMsgReplyHtml = function(dom, dataId) {
        var options = {
            openId : sun.util.parseToInt(dataId)
        };
            ...
            sun.util.each(json, function(v, i) {
                ...
                if (v.reply.length > 0) {
                    sun.util.each(v.reply, function(w, j) {
                        json[i].reply[j].createTime = sun.util.formatTime('yyyy-MM-dd hh:mm:ss', w.createTime);
                    });
                }
            });
    ...
    wcm_message.initList = function(nId, currentPageNum) {
        ...
        sun.ajax.getJSON(listURL, params, function(jsonData) {
            ...
            $('#DOM_pgn').bootstrapPaginator({
                ...
                totalPages: json.pagination.pageSize,
    ...
    wcm_message.bindEvent = function() {
        ...
        $('#msg_all').on('click', '.msg-reply-save', function() {
            ...
            var _replyId = correntDOM.children().attr('id'),
                _msgId = $(this).parents('.msg-wrapper').attr('data-id'),
                _openId = $(this).parents('.msg-wrapper').attr('data-openId'),
                txt = '';
            ...
            if (TAG_msgType === '1') { // msg type is txt
                ...
                if (txt.length > 0) {
                    sun.ajax.post({
                        url : wcm.getApiURL("saveKeywordReplayText"),
        ...
        $('#msg_all').on('click', '.msg-detail', function(e) {
            ...
            var dataId = $(this).parents('.msg-wrapper').attr('data-id');
            ...
            if (tag.indexOf('detail') > -1) {
                ...
                wcm_message.setMsgReplyHtml(replyList, dataId);
            } else {
                $(this).text('展开');
